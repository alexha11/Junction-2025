...
const HeroHeader: FC<Props> = ({ state, schedule, alertsCount }) => {
	...
	return (
		<section className="relative overflow-hidden rounded-[32px] border border-white/10 bg-gradient-to-brp-8 glass-card ">
			<div className="" />
			...
			<div className="mt-6 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
				<div className="space-y-4">
					<div>
						...
						<h1 className="text-3xl font-semibold text-white sm:text-4xl">
							Operational resilience through transparent,
							multi-agent decisions.
						</h1>
					</div>
					<p className="max-w-3xl text-base text-slate-300">
						Weather, inflow, market, and physics agents collaborate
						via MCP tooling, feeding FastAPI services that publish
						actionable recommendations every 15 minutes. Operators
						always see the evidence, can audit justifications, and
						override safely.
					</p>
				...
				<div className="rounded-3xl border border-white/10 bg-black/40 px-6 py-4 text-sm text-slate-200">
					...
					<p className="mt-3 text-xs text-slate-400">
						Recommendation payloads, justification text, and manual
						overrides are retained for audit readiness.
					</p>
				</div>
			</div>
		</section>
