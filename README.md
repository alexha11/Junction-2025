...
- Python 3.12 recommended (backend + agents) with FastAPI, Pydantic, APScheduler, SQLAlchemy, asyncpg, Redis, httpx, and OpenAI Agents SDK placeholders.
- Node.js 20+ for Vite/React frontend (React 18, React Query, Tailwind, Recharts, Zustand).
- Docker Compose for local orchestration (backend, frontend, Postgres, Redis).
- OPC UA + MCP (FastMCP) for the digital twin server/client tooling.
...
- **MCP bridge**
  ...
  ```
- **Test clients**: `digital-twin/test-clients/opcua_client.py` and `mcp_client.py` demonstrate bidirectional calls.

## Docker Orchestration

`docker-compose.yml` brings up backend (Uvicorn), frontend (Nginx + Vite build), Postgres 16, and Redis 7.
