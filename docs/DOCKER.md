...
| Postgres container restart loop | Port `5432` may already be in use on the host. Set `POSTGRES_PORT=55432` and update `DATABASE_URL` to match, then re-run `docker compose up`. |
