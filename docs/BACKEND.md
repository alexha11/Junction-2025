...
3. **Background optimization**: The scheduler executes `_optimize()`, awaiting the latest schedule and logging the computed payload. Future work: persist schedules, push WebSocket/SSE updates, and trigger alert generation.

## Logging & Diagnostics

- Centralized logging lives in `app/logging_config.py` and is wired up during FastAPI startup. Control verbosity via the `LOG_LEVEL` environment variable (defaults to `INFO`).
- Routers, the `AgentsCoordinator`, and the `OptimizationScheduler` now emit informative events whenever they serve HTTP traffic, call agents, or compute new recommendations.
- For a ready-made troubleshooting checklist plus `curl` examples, see `backend/DEBUGGING.md`.
